...
mkfifo /tmp/mc-stdin
tail -f /tmp/mc-stdin | ./start.sh &
minecraftPid=$!
cd ..

# Wait for server to start
echo "Server starting! Waiting 20m for someone to join"
...
while [[ "$players" != 0 ]]; do
  echo "Get players"
  players=$(curl -s "https://mcapi.xdefcon.com/server/${PUBLIC_IP}/players/json" | sed -e 's/[{ }]/''/g' | awk -v RS=',' -F: '{print $2}')
